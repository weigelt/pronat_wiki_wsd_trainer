...
/**
 * This class represents a Trainer, that can handle extracting training data
 * from lines from Wikipedia dumps
 *
 * @author Jan Keim
 *
 */
public class WikiWSDTrainer extends Trainer {
	...
	private static Pattern buildPattern() {
		return Pattern.compile("\\[\\[([\\w\\s\\-\\.\\(\\)']+\\|)?([\\w\\s\\-\\.']+)\\]\\]", Pattern.UNICODE_CHARACTER_CLASS);
	}

	/*
	 * (non-Javadoc)
	 *
	 * @see im.janke.wsdTrainer.Trainer#addTrainingData(java.lang.String,
	 * java.lang.String)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see Trainer#addTrainingData(java.lang.String)
	 */
	@Override
	public void addTrainingData(String line) {
		...
		for (CoreMap sentence : document.get(SentencesAnnotation.class)) {
			...
			for (Integer index : senseIndices) {
				...
				String meaning = disambiguations.get(word.toLowerCase()).poll();
				if (meaning == null) {
					...
					if (wordPos.startsWith("NN")) {
						for (String disambiguationPhrase : wordSet) {
							if (disambiguationPhrase.contains(word.toLowerCase())) {
								meaning = disambiguations.get(disambiguationPhrase).poll();
							}
						}
					}
	 ...
	 *
	 * @param matcher
	 *            Matcher that will be used to find the disambiguations along with
	 *            their representation
	 * @return Mapping of Disambiguation to list of representations for that
	 *         disambiguation in order of their appearance within the text.
	 */
	private static Map<String, ArrayDeque<String>> getDisambiguationMap(Matcher matcher) {
		...
		while (matcher.find()) {
			...
			String meaning = (matcher.group(1) == null) ? word : matcher.group(1).replace("|", "").toLowerCase();
	 ...
	 * @return List of indices where the disambiguation words are within the
	 *         sentence
	 */
	private static List<Integer> getDisambiguationIndices(Map<String, ArrayDeque<String>> disambiguations, List<CoreLabel> tokens) {
					...
					outer: for (String disambiguationPhrase : wordSetCopy) {
						if (disambiguationPhrase.contains(word.toLowerCase())) {
							...
							for (int existingSenseIndex : senseIndices) {
								String tmp_word = tokens.get(existingSenseIndex).get(TextAnnotation.class);
	...
	/**
	 * Creates the Surrounding Word Attribute Values for the provided instance.
	 * Surrounding words are left 3 words and right 3 words, stopwords and unwanted
	 * words are filtered. For each word its POS is also evaluated and put into the
	 * instance.
	 *
	 * @param tokens
	 *            Tokens of the sentence
	...
	private void addSurroundingWordsAttributes(List<CoreLabel> tokens, Integer index, Instance instance) {
		...
		for (int i = 1; i <= 3; i++) {
			...
			if (rightIndex < tokens.size()) {
				...
				rightLemma = ((rightLemma != null) && (!rightLemma.startsWith("#"))) ? rightLemma.toLowerCase() : "NONE";
	...
	/**
	 * Finds the Noun and Verb that are each left and right next to the actual
	 * disambiguation word
	 *
	 * @param tokens
	 *            Tokens of the sentence
	...
	private void addLeftAndRightNounsAndVerbs(List<CoreLabel> tokens, Integer index, Instance instance) {
		...
		for (int i = 1; i < tokens.size(); i++) {
			...
			if (!leftNN.equals("NONE") && !leftVB.equals("NONE") && !rightNN.equals("NONE") && !rightVB.equals("NONE")) {
				break;
			}
		}
	...
	private static String[] getMostFrequentWords(Annotation document) {
		HashMap<String, Integer> words = new HashMap<>();
		for (CoreLabel token : document.get(TokensAnnotation.class)) {
			String wordLemma = token.get(LemmaAnnotation.class).toLowerCase();
			if (Stopwords.isStopword(wordLemma) || ClassifierService.filterWords.contains(wordLemma)
					|| ClassifierService.additionalFilterWords.contains(wordLemma) || wordLemma.matches("^\\+?[\\d]+.*")) {
	...
	private static String lemmatizePOS(String pos) {
		switch (pos) {
		case "JJR":
		case "JJS":
			return "JJ";
