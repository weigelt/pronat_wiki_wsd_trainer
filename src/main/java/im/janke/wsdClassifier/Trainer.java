...
public abstract class Trainer {
	...
	Trainer(Classifier classifier, Instances trainingSet) {
	...
	/**
	 * Processes the line and adds found instances to the training data
	 * ({@link #trainingSet}
	 *
	 * @param line
	 *            Line that should be processed
	...
	/**
	 * Builds the classifier. Before, do necessary preparation like the
	 * StringToNominal-Filtering, if it was not done before. Also creates the
	 * classifier (and sets necessary options).
	 */
	public void buildClassifier() {
		try {
			if (!trainingSet.attribute(0).isNominal()) {
	...
	private static Instances stringToNominalFiltering(Instances instances) throws Exception {
	...
	static Instances filterStringToNominal(Instances instances, String range) throws Exception {
		StringToNominal stringFilter = new StringToNominal();
	...
	Instances filter(Instances instances, Filter filter) throws Exception {
		if (removeUnique) {
	...
	private static Instances filterOutUniqueInstances(Instances instances) {
	 ...
	 */
	Optional<Evaluation> evaluateWithInstances(Instances trainingSet) {
	...
	/**
	 * Evaluates the classifier and returns the
	 * {@link weka.classifiers.evaluation.Evaluation}. A 10-fold cross validation is
	 * performed.
	 *
	 * @return Optional of an Evaluation. The Optional is <code>null</code> when
	 *         there was an exception within the evaluation or the Trainer was not
	 *         prepared (use {@link #buildClassifier()} beforehand!)
	 */
	...
	/**
	 * Evaluates the classifier and returns the
	 * {@link weka.classifiers.evaluation.Evaluation}. If the provided boolean is
	 * set, then a 10-fold cross validation is performed. Otherwise evaluation is
	 * done with the training data.
	 *
	...
	public Optional<Evaluation> evaluate(boolean crossValidation) {
		if (!trainingSet.attribute(1).isNominal() || !trainingSet.attribute(trainingSet.numAttributes() - 1).isNominal()) {
		...
		if (isBuild) {
			return (crossValidation) ? evaluateCrossValidationFolds(trainingSet, 10) : evaluateWithInstances(trainingSet);
		} else {
			return Optional.empty();
		}
	}
	 ...
	 * @return the evaluation object or null, if there was an exception
	 */
	Optional<Evaluation> evaluateCrossValidationFolds(Instances trainingSet, int folds) {
		Evaluation eval = null;
		try {
			eval = new EfficientEvaluation(trainingSet);
	 ...
	 *
	 * @return false if there are no instances in the training set. If there are
	 *         some, returns true
	 */
	...
	public synchronized String dataSummaryString() {
		...
		for (int i = 0; i < trainingSet.numAttributes(); i++) {
			...
			String s = String.format("%6d / %3d%%  ", as.missingCount, Math.round((100.0 * as.missingCount) / as.totalCount));
			strBuilder.append(s);
	...
	/**
	 * Merges two instances together, also copying string and nominal attributes.
	 * Relation name of the first instance will be taken.
	 *
	 * See:
	 * https://stackoverflow.com/questions/10771558/how-to-merge-two-sets-of-weka-instances-together
	 *
	 * Use this, not {@link Instances#mergeInstances(Instances, Instances)} because
	 * the other one merges only if datasets are the same size and is primarily to
	 * merge different attribute values (for kinda same data set)
	 *
	 * AWARE: Works best, when there are no nominal attributes, only string
	 * atrributes. If there are some, then they must be the same for both instances!
	 *
	 * @param data1
	 ...
	 */
	private static Instances mergeInstances(Instances data1, Instances data2) throws Exception {
	...
	/**
	 * Same as {@link #mergeInstances(Instances, Instances)} but this time the
	 * second instance gets inserted into the first instance instead of creating a
	 * new one.
	 *
	 * @param data1
	 *            first data instances
	...
	public static void mergeInstancesToFirst(Instances data1, Instances data2) throws Exception {
		...
		while (source.hasMoreElements(instances)) {
			...
			for (int i = 0; i < asize; i++) {
				if (strings_pos[i]) {
					data1.instance(data1.numInstances() - 1).setValue(i, instance.stringValue(i));
				}
			}
		}
	}
