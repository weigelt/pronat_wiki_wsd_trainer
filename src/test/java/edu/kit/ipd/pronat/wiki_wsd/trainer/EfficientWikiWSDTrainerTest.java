package edu.kit.ipd.pronat.wiki_wsd.trainer;
...
import edu.kit.ipd.parse.wiki_wsd.trainer.EfficientWikiWSDTrainer;
import edu.kit.ipd.parse.wiki_wsd.trainer.Trainer;
import edu.kit.ipd.parse.wiki_wsd.trainer.WikiWSDTrainer;
...
/**
 * Tests to check if the efficient trainer (and classifier) is still performing
 * the same.
 * 
 * NOTE: Due to a weird behavior of handling attribute weights in WEKA, the
 * normal WikiWSDTrainer and its corresponding base NaiveBayes-classifier
 * performs actually wrong, as they don't factor in the attribute weights
 * properly (although set). This is why this test class is currently disabled.
 * 
 * @author Jan Keim
...
public class EfficientWikiWSDTrainerTest {
	...
	private void testClassificator(Instances instances) {
		...
		for (int i = 0; i < testInstances.size(); i++) {
			...
			Assert.assertEquals("Different classification for instance " + i + "! Correct was " + corr + ".", base, test);
		}
	}

	/**
	 * Integration test that trains with the {@link EfficientWikiWSDTrainer} and
	 ...
	 * They are tested on a set of training data with many classes (LIMIT / 4).
	 *
	 */
	@Test
	...
	/**
	 * Tests if the efficient Trainer classifies the same as the original trainer.
	 * They are tested on a set of training data with only 10 different classes.
	 */
	@Test
	 ...
	 * Tests if the efficient Trainer classifies the same as the original trainer.
	 * They are tested on a set of training data with only 10 different classes
	 * where the attributes are all specific for one class
	 */
	@Test
	...
	public void testClassificator_dataHandling() {
		...
		for (int i = 0; i < size_orig; i++) {
			doubleArray[i] = instances.get(i).toDoubleArray();
		}
